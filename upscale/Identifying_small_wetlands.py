...
def calculate_areas(data_with_neighbours,delta_neighbour,binary_image,thresholds_list):
    ...
    return area_stats
def get_tile_edge_pixels(tif_data,yoff,xoff,delta_neighbour,dir,tiff_lst,year):
    ##########################################
...
def identifying_small_wetlands(lats,lons,start_year,end_year,thresholds_list,dir,output_dir):
    for year in range(start_year,end_year+1):
            ...
            tiff_lst = [
                ...
                if fnmatch.fnmatch(f, f"GWL_FCS30D_{year}Maps_*.tif")
            ]
            for tiff in tqdm(tiff_lst[:]):
                ...
                data_with_neighbours=get_tile_edge_pixels(tif_data, yoff, xoff, delta_neighbour, dir,tiff_lst,year)
