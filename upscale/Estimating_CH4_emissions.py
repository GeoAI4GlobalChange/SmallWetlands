...
def global_wetland_CH4_emissions(start_year,thresholds_list,input_wetland_dir,input_CH4_dir,output_dir):

    dir = input_wetland_dir
    target = 'extent'
    nc_file = dir + f'GWL_global_0.5degree.nc'
    data = Dataset(nc_file, mode='r')
    extent_all = np.array(data.variables[target][:])##shape: thres*time*lat*lon
    ...
    CH4_flux=np.repeat(CH4_flux,extent_all.shape[0],axis=0)
    ...
    nc_fid2.createDimension('time', CH4_flux.shape[1])
    nc_fid2.createDimension('thres', CH4_flux.shape[0])
    ...
    var_save = nc_fid2.createVariable('wetland_CH4_emissions', "f4", ("thres", "time","lat", "lon",))
    var_save.units = 'gCH4 m-2 day-1'
    time_day.units = "days since 1993-08-01 00:00:00.0"
    time_day.calendar = "gregorian"
    dates_all=[]
    year_num=int(CH4_flux.shape[1]/52)
    for year_idx in range(start_year,start_year+year_num):
...
if __name__=='__main__':
    ...
    # the directory of wetland data: GWL_global_0.5degree.nc
    ...
    # The input drivers are provided under the 'samples' folder;
    # The estimated CH4 fluxes are provided under the "output" folder: FCH4_upscale_ML.nc
    input_CH4_dir = r'../output/'
    # the output directory to save the estimated gridded CH4 emissions
    output_dir = f'../output/'
    global_wetland_CH4_emissions(start_year,thresholds_list,input_wetland_dir,input_CH4_dir,output_dir)